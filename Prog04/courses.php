...
function printListing($apiCall) {
    ...
    if (!($obj->courses == null)) {
        ...
        // Sort the array of course objects
        $listings = sortListings($listings);
        
        // Continue as normal
        foreach ($listings as $course) {    
            if( $_GET['day'] != "" ) {
                if(strcmp(substr($course->meetingTimes[0]->days,0,1), $_GET['day'])) {
                    continue;
                }
            }
...
/*
 * sortListings takes an array of JSON objects the array is sorted by
 * the days of the week using an associative array holding the priority
 * of each day. The array is returned after sorted.
 */
function sortListings($listings) {
    $length = sizeof($listings);
    $sorted = FALSE;
    // Associative Array to define priority
    $priority = array("M"=>1,"MW"=>2,"T"=>3,"TR"=>4,"W"=>5,"R"=>6,"F"=>7,""=>8);
    while(!$sorted) {
        $sorted = TRUE;
        for($i = 0; $i < $length - 1; $i++ ) {
            if($priority[$listings[$i]->meetingTimes[0]->days] > $priority[$listings[$i + 1]->meetingTimes[0]->days] ) {
                $sorted = FALSE;
                $temp = $listings[$i];
